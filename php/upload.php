...
$category_goods = isset($_POST['category_goods']) ? $_POST['category_goods'] : '';
$kind_goods = isset($_POST['kind_goods']) ? $_POST['kind_goods'] : '';
...
$sql = "INSERT INTO `goods` (`name_goods`, `price_goods`, `count_goods`, `category_goods`, `kind_goods`, `description`) VALUES (?, ?, ?, ?, ?, ?)";
$stmt = $mysql->prepare($sql);
$stmt->bind_param("siiiss", $name, $price, $count, $category_goods, $kind_goods, $description);
...
if ($stmt->execute()) {
    ...
    if (!empty($images['name'][0]) && $images['error'][0] === UPLOAD_ERR_OK) {
        ...
        echo "Ошибка: файлы не были загружены.";
    }
